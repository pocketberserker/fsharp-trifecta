...
module AppliedOnce =
  let inline (++) (a: #AppliedOnce<'T, 'A>) (m: #_1<'T, 'B>) = a.Map2(m, id)
  let inline (>>.) (a: #AppliedOnce<'T, 'A>) (m: #_1<'T, 'B>) = a.Map2(m, snd)
  let inline (.>>) (a: #AppliedOnce<'T, 'A>) (m: #_1<'T, _>) = a.Map2(m, fst)
...
type Alternating<'T, 'A>() =
  ...
  member inline this.Many() = this.LiftAl(this.Some()).OrElse([])
  member this.Some() = this.Map2(this.Many(), fun (a, b) -> a :: b)
  member inline this.SkipMany() = this.LiftAl(this.SkipSome()).OrElse(())
  member this.SkipSome() = this >>. this.SkipMany()
  member inline this.SepBy(sep) = this.LiftAl(this.SepBy1(sep)).OrElse([])
  member this.SepBy1(sep) =
    this.Map2(this.LiftAl(this.LiftAl(sep) >>. this.Self()).Many(), fun (a, b) -> a :: b)
  member inline this.SkipSepBy(sep) = this.LiftAl(this.SkipSepBy1(sep)).OrElse(())
  ...
  member inline this.Chainl(op: _1<'T, 'A * 'A -> 'A>) =
  ...
  member inline this.EndBy1(sep) = this.LiftAl(this .>> sep).Some
  member inline this.endBy(sep) = this.LiftAl(this .>> sep).Many
  member inline this.SkipEndBy1(sep) = this.LiftAl(this .>> sep).SkipSome
  member inline this.SkipEndBy(sep) = this.LiftAl(this .>> sep).SkipMany
  member inline this.SepEndBy1(sep) =
  ...
  member inline this.SkipSepEndBy1(sep) =
  ...
  abstract member LiftF: _1<'T ,'B> -> Filtered<'T, 'B>
  abstract member WithFilter: ('A -> bool) -> _1<'T, 'A>
  member inline this.FilterMap(f: 'A -> _ option) =
    this.LiftF(this.LiftF(this.Map(f)).WithFilter(Option.isSome)).Map(Option.get)
  member inline this.Filter(p) = this.WithFilter(p)
